...
class Vec2d(object):
    ...
    def argument(self, radians=False):
        ...
        arg_in_rad = math.acos(Vec2d(0, 1) * self / self.norm())
        if radians:
            return arg_in_rad
    ...
    def normalize(self):
        ...
        for value in self:
            if value:
                normed.append(value/norm)
            else:
                normed.append(value)
        return self.__class__(*tuple(normed))
    ...
    def matrix_mult(self, matrix):
        ...
        product = tuple(Vec2d(*row) * self for row in matrix)
    ...
    def dot(self, vector):
        ...
        """
        if not isinstance(vector, Vec2d):
    ...
    def length(self, value):
        norm = self.normalize()
        self.values = tuple(x * value for x in norm)
